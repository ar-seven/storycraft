    ...
    if prespective == "third":
        completion = openai.ChatCompletion.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": f"Generate a delightful 4 paragraph  children's story with an enchanting title about {prompt} that shares an important moral lesson. Our stories are meant to inspire and educate, so please choose a topic that's fun, safe, and suitable for kids. If the topic is irrelevant or inappropriate, share a special story with another topic instead. Also create an image prompt for the story that mentions gender, physical features of characters and environment. Return the answer as a JSON object.Seperate paragraphs of story using \\n\\n. {{Title:, Prompt:, Story:,}}",
                }
            ],
        )
    else:
        ...
        completion = openai.ChatCompletion.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": f"Generate a delightful 4 paragraph  children's story with an enchanting title about {prompt} that shares an important moral lesson. The story should be told entirely from a first-person perspective. Please use 'I,' 'Me,' and 'Myself' throughout the story, as if I am the hero of the tale. Our stories are meant to inspire and educate, so please choose a topic that's fun, safe, and suitable for kids. If the topic is irrelevant or inappropriate, share a special story with another topic instead. Also, create an image prompt for the story that mentions gender, physical features of characters, and the environment.Return the answer as a JSON object.Seperate paragraphs of story using \\n\\n. {{Title:, Prompt:, Story:,}}.",
                }
            ],
        )
